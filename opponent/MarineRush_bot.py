    ...
    async def on_step(self, iteration: int):
        ...
        if self.strategy_num == 1:
            if ccs.exists:  # 사령부가 하나이상 존재할 경우
                ...
                if self.time < 17:                   
                    if self.can_afford(UnitTypeId.BANSHEE) and self.units(UnitTypeId.BANSHEE).amount == 0:
                    ...
                    elif self.can_afford(UnitTypeId.RAVEN) and self.units(UnitTypeId.RAVEN).amount == 0:
                        if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0:
        ...
        for battle in battlecruisers:
            ...
            # if self.known_enemy_units(UnitTypeId.SIEGETANKSIEGED).exists:
            #     enemy_battle = self.known_enemy_units(UnitTypeId.SIEGETANKSIEGED)
            ...
            if self.known_enemy_units(UnitTypeId.VIKINGFIGHTER).exists:
                enemy_battle = self.known_enemy_units(UnitTypeId.VIKINGFIGHTER)
                if enemy_battle.exists:
                    actions.append(battle(AbilityId.YAMATO_YAMATOGUN, target = enemy_battle[0]  ))
        ...
        for battle in battlecruisers:
            ...
            if self.battle_has_matrix == 1:
            ...
            elif self.time - self.Nuke_calldown_time > 10 and self.Nuke_calldown == True:
                ...
                if self.time < burst_time :
                    if self.known_enemy_units.exists:
                        ...
                        if battle.distance_to(enemy) < 20:
                            if enemy.is_cloaked:
                                ...
                                if self.known_enemy_units(UnitTypeId.VIKINGFIGHTER).exists:
                                    ...
                                    if battle.distance_to(enemy_viking) < enemy_viking.radius + battle.radius + 10:
                                        actions.append(battle.move(enemy_viking.position.towards(battle,10)))
                                    else:
                            ...
                            if ally_cc[0] < 64:
                                actions.append(battle.attack(target + (14, 0)))
                            else:
                                actions.append(battle.attack(target + (-14, 0)))   
                    else:
                        if ally_cc[0] < 64:
        ...
        if False:   # self.strategy_num == 3 or self.enemy_counter[UnitTypeId.SIEGETANKSIEGED] + self.enemy_counter[UnitTypeId.SIEGETANK] >= 1:
            ...
            for marine in marines:
                ...
                if self.time - self.Nuke_calldown_time > 10 and self.Nuke_calldown == True:
                    ...
                    if ally_cc[0] < 64:
                        actions.append(marine(AbilityId.ATTACK, target =  target + (5, 0)))
                    else:
                        actions.append(marine(AbilityId.ATTACK, target =  target + (-5, 0)))
        ...
        for raven in ravens:
            enemys = self.known_enemy_units
            if self.known_enemy_units.exists:           
                if self.known_enemy_units.exists and self.time > 90:
                    if self.known_enemy_units.amount >= 5:
                        actions.append(raven(AbilityId.EFFECT_ANTIARMORMISSILE, target = self.known_enemy_units.closest_to(self.known_enemy_units.center)))
                    else:
